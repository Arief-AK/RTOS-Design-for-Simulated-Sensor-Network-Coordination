...
class SchedulerComparisonTest : public ::testing::TestWithParam<std::function<std::unique_ptr<Scheduler>()>> {};

// Instantiate the test with different schedulers
INSTANTIATE_TEST_SUITE_P(SchedulerTypes, SchedulerComparisonTest,::testing::Values(
    []() {return std::make_unique<RoundRobinScheduler>(2); },
    []() {return std::make_unique<PriorityScheduler>(); },
    []() {return std::make_unique<EDFScheduler>(); }
));

TEST_P(SchedulerComparisonTest, CompleteAllTasks){
